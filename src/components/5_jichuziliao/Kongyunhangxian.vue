  ...
  <div class="index">
    <div class="top">
      ...
      <div class="t-tit2">
        ...
        <div class="tt2-right">
          <el-button :disabled="_5_qx.indexOf('增加') == -1" @click="addDingdan" type="success">创建航线</el-button>
        </div>
      </div>
    </div>
    <div class="content">
      <div class="myTable">
        <vxe-table :data="tableData">
          ...
          ></vxe-table-column>
          <vxe-table-column
            field="port_departureid"
            title="起运港代码"
          ></vxe-table-column>
          <vxe-table-column field="port_destinationid" title="目的港代码"></vxe-table-column>
          ...
          <el-row>
            <el-col :span="20">
              <el-form-item label="航线代码：">
                <el-input size="small" v-model="addForm.route_code"></el-input>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="20">
              <el-form-item label="起运港代码：">
                <el-select
                  filterable
                  size="small"
                ...
                >
                  <el-option
                    ...
                    :key="item.id"
                    :label="`${item.three_charcode}-${item.english_name}-${item.chinese_name}`"
                    :value="item.id"
                  >
                    <div style="display: flex; justify-content: space-between">
                      <span style="color: #8492a6; font-size: 12px;margin:0 12px">{{item.three_charcode}}</span>
...
export default {
  computed: {
    ...mapState(["headerTit", "_5_8Page", "_5_8PageSize",'_5_qx']),
  },
  watch: {
    _5_8Page: function (page) {
      this.$store.commit("_5_8Page", page);
        this.getData();
    },
    _5_8PageSize: function (pageSize) {
      this.$store.commit("_5_8PageSize", pageSize);
        this.getData();
    },
  },
  data() {
  ...
  methods: {
    async getData() {
      const res = await this.$api.airroute_list({
        page: this._5_8Page,
        pagesize: this._5_8PageSize,
    ...
    async tabDel(row) {
      const res = await this.$api.del_airroute({ id: row.id });
    ...
    async AddOnSubmit() {
      if (this.isAdd) {
        const res = await this.$api.create_airroute({
          ...this.addForm,
        });
        console.log(res);
        this.$message(res.msg);
        ...
        const res = await this.$api.update_airroute({
          ...this.addForm,
          id: this.id,
        });
        console.log(res);
    ...
    xindingdanHandleSizeChange(val) {
      ...
      this.$store.commit("_5_8PageSize", val);
    },
    xindingdanHandleCurrentChange(val) {
      console.log(`当前页: ${val}`);
      this.$store.commit("_5_8Page", val);
    },
  },
};
</script>
